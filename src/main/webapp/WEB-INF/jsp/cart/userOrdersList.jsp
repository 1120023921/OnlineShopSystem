...
function getAllOrders(pages) {

	$.ajax({
				"url" : path+'/orders/getAllUserOrders.do',
				data : {
					pages : pages,
					rows : 5
				},
				...
				"success" : function(data) {
					...
					for (var i =0;i<data.size;i++) {
						$("#orderList").html($("#orderList").html()+'<a class="weui_cell" href="${pageContext.request.contextPath}/userOrdersDetail.do?id='+data.list[i].id+'">'
							+'<div class="weui_cell_bd weui_cell_primary">'
							+'<p>'+getMyDate(data.list[i].time)+'</p>'
							+'</div>'
							+'<div class="weui_cell_ft">详细信息</div>'
							+'</a>');
					}

					if(data.pageNum>1){
						$("#pageBar").html('<li onclick="getAllOrders('+(data.pageNum-1)+')"><a href="#">&laquo;</a></li>');
					}
					...
					for(var i = firstPage;i<=lastPage;i++){
						if(i==data.pageNum){
							$("#pageBar").html($("#pageBar").html()+'<li onclick="getAllOrders('+i+')" class="active"><a href="#">'+i+'</a></li>');
						}else {
							$("#pageBar").html($("#pageBar").html()+'<li onclick="getAllOrders('+i+')"><a href="#">'+i+'</a></li>');
						}
					}
					
					if(data.pageNum<data.pages){
						$("#pageBar").html($("#pageBar").html()+'<li onclick="getAllOrders('+(data.pageNum+1)+')"><a href="#">&raquo;</a></li>');
					}
...
<body>
	<div class="page slideIn home">
		<div>
			<br>
			<h1 class="page_title">用户订单管理</h1>
			<p class="page_desc">LA LUNA</p>
		</div>
		<div class="weui_cells weui_cells_access" id="orderList">
		...
		<a href="${pageContext.request.contextPath}/personCenter.do"
